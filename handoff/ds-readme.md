...
| `components/forms/` | Input, Textarea, Select, Combobox (grupos + trailing), Checkbox, CheckboxGroup, Radio, RadioGroup, Switch, Fieldset, FormRow, Calendar (single/range, mês/ano, marcadores, dias desabilitados), DatePicker, DateRangePicker, TimePicker, TimeInput (máscara HH:mm + stepper) |
| `components/display/` | Card, Badge, Tag, Avatar, AvatarGroup, Accordion |
| `components/navigation/` | Tabs, Breadcrumb, Pagination, Dropdown, Stepper, CommandPalette (⌘K), WorkspaceSwitcher, CreateWorkspaceDialog, SidebarGroup, AppSidebar, BottomNav |
| `components/feedback/` | Alert, Toast, ToastStack, Dialog, Drawer, Tooltip, Progress, Spinner, Skeleton, CookieBanner, BottomSheet |
| `components/data/` | Table, DataTable (sort, seleção, sticky), Stat, EmptyState, PersonCell, DiffCard |
| `components/layout/` | Stack, Inline, Grid, Container, Separator, PageHeader, AppShell, ActionBar |
| `components/primitives/` | Popover, Portal, VisuallyHidden |
| `components/scheduling/` | CalendarView (agenda dia/semana/mês), SlotPicker (booking público), TimeZonePicker (IANA + busca), WeeklyScheduleEditor (disponibilidade semanal + exceções), RecurrenceSelector (recorrência com resumo), SegmentedRing (progresso decomposto) |
| `components/system/` | ThemeProvider + useTheme, ToastProvider + useToast |
| `components/files/` | FileUpload (multiupload com progresso), FileManager (gestor de arquivos), Dropzone (alvo de soltar) |
...
| `ui_kits/website/` | UI kit: site do Lyra — landing (lyra-ds.dev) + docs (docs.lyra-ds.dev), 100% open source, sem pricing |
