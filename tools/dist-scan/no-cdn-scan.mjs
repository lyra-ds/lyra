...
// │ The Phase 3 build-job dist scan (plan 03-08) AND the 03-09 scratch-fixture bundle   │
// │ scan BOTH invoke this exact script, so their rule sets are byte-identical by         │
// │ construction. The 03-09 fixture bundles react-dom, so its output deterministically   │
// │ contains the `https://react.dev/` decoder literal — which is why that prefix is on   │
// │ the allowlist here even though @lyra-ds/react keeps react external (no such literal   │
// │ appears in its own dist). Do NOT fork these constants into a second scanner.         │
// └────────────────────────────────────────────────────────────────────────────────────┘
//
// Usage: node tools/dist-scan/no-cdn-scan.mjs <dir> [<dir> ...]
...
const FORBIDDEN_PACKAGE_STRINGS = ['lucide-static'];
  ...
  for (const file of files) {
    ...
    for (const pkg of FORBIDDEN_PACKAGE_STRINGS) {
      if (code.includes(pkg)) {
        violations.push(`${file}: forbidden package reference "${pkg}"`);
      }
    }
    ...
    for (const match of code.match(URL_LITERAL) ?? []) {
      ...
      violations.push(
        cdn
          ? `${file}: known CDN host "${cdn}" in URL ${match}`
          : `${file}: non-allowlisted URL literal ${match}`,
      );
    }
  }
}
