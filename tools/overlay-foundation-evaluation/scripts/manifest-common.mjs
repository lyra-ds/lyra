...
export const EXPECTED_TOOLCHAIN = Object.freeze({ node: '24.18.0', pnpm: '11.13.1' });
...
export function requireMatchingIncumbent(lyraRevision, incumbentCharacterization) {
...
function decodeUtf8(bytes, description) {
  if (bytes.length >= 3 && bytes[0] === 0xef && bytes[1] === 0xbb && bytes[2] === 0xbf) {
    throw new Error(`${description} must not contain a BOM`);
  }
  try {
    return utf8Decoder.decode(bytes);
  } catch (error) {
    throw new Error(`${description} must be valid UTF-8`, { cause: error });
  }
}

async function readIncumbentCharacterization(path) {
  try {
    return JSON.parse(decodeUtf8(await readFile(path), 'incumbent characterization'));
  } catch (error) {
...
export async function writeCandidateManifest({
  ...
  const manifest = createManifest({
    lyraRevision,
    incumbentCharacterization: await readIncumbentCharacterization(resolve(incumbentPath)),
  });
  assertValidManifest(manifest);
  const bytes = canonicalJson(manifest);
  await writeFile(output, bytes, { flag: 'wx', mode: 0o600 });

  const reread = JSON.parse(decodeUtf8(await readFile(output), 'written manifest'));
  assertValidManifest(reread);
  if (canonicalJson(reread) !== bytes) throw new Error('written manifest is not canonical');
  return reread;
}

export function parseManifestArguments(argv, scriptName) {
  const usage = `usage: ${scriptName} --revision <sha> --incumbent <path> --output <path>`;
