# Shared brief — @lyra-ds/alpine wave E (lots E1–E3 sit on top of this)
...
- `packages/alpine` — the package you are working on. Waves 1, B, C and D
  merged on main: TWENTY-ONE shipped bindings plus the `theme` store
  (dropdown, dialog, drawer, tabs, accordion, tooltip, popover, code-block,
  cookie-banner, sidebar-group, segmented-control, workspace-switcher,
  bottom-sheet, table-of-contents, time-input, file-upload, file-manager,
...
- `packages/react` — the SOURCE OF TRUTH for behavior. Each Alpine
  ...
  component's `.tsx` and its `.browser.test.tsx` (and `*.v12.browser.test.tsx`
  where present) before writing code.
...
put it. React callbacks become custom events via
`this.$dispatch('lyra:...')` (bubbling, with a detail payload).

Wave-E specific model: these are APG `aria-activedescendant` components.
DOM focus stays FIXED on the search input while the active option changes;
options are rendered `tabIndex=-1` with stable ids derived from the option's
ORIGINAL index in the data array (never the filtered index). The
`--active` class and `activeIndex` track positions in the FILTERED
collection. Options/groups are DATA-driven: arrays passed to the factory,
rendered via `x-for` over derived getters, with per-item helper methods for
the consumer template (binding objects cannot see `x-for` scope — same
decision as calendar/file-upload).
</context>
...
- Shared logic lives in `packages/alpine/src/internal/` as plain functions:
  ...
  `when-visible.ts`, `test-axe.ts`, `tooltip-placement.ts`, `scroll-spy.ts`,
  `date-utils.ts` already exist — REUSE them; do not fork variants. Lot E1
  CREATES `internal/active-descendant.ts`; lots E2/E3 reuse it.
...
8. Reactive state written from inside a `$watch` callback can fail to
   schedule dependent `x-show` effects during the same flush (the effect
   looks dead). Derive such values INSIDE the consuming effect — read the
   reactive deps in a method the binding calls — instead of holding
   watcher-written state.
9. In tests, every x-show transition must LAND (assert it with
   `vi.waitFor` on real visibility) before triggering the next one: the
   rAF-deferred reveal of a previous step can arrive after a later hide
   and stick the element wrong. Never assert mere existence of an
   x-show-gated element — it exists (hidden) from the first render.
10. The x-model/entangle channel SERIALIZES non-JSON values (Dates became
    ISO strings in wave D). Any modelable state that is not a plain
    string/number/boolean/array/object must be normalized ON READ.
11. Chaining `x-modelable` through nested components with the SAME property
    name entangles the component with itself. When a fixture or canonical
    template must chain a modelable through a wrapper scope, alias it with
    a get/set `x-data` scope layer (see the date-picker JSDoc template).
12. `<template x-if>` mounts only ONE root element — multi-root content
    needs a wrapper or separate templates.

Reference implementations, merged and green — copy their structure before
inventing your own: `src/dropdown.ts` (outside mousedown, flip placement),
`src/workspace-switcher.ts` (listbox popover, pendingFocus),
`src/bottom-sheet.ts` + `src/dialog.ts` (presence/`mounted`, focus trap,
scroll lock, opener restore), `src/calendar.ts` (data-driven `x-for` with
per-item helper methods, canonical JSDoc template), `src/time-picker.ts`
(listbox with clamp arrows and scroll-on-open), with their test suites.
</lessons_from_waves_1_to_D>
