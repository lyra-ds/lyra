...
- [ ] 37. CreateWorkspaceDialog public lifecycle and migration — documentation/medium, after26. Scope exactly en/pt-BR create-workspace-dialog.mdx. Explain actual request/result/signal, validation/pending/error/retry/cancel and controlled close, migration from plain/void callback; current complete typed code, two existing live examples only; preserve static HTML panel and support metadata. Accept current MDX compile/snippet types/stack/format, exact public snippet native behavior and independent review.
