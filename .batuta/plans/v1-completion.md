...
- [ ] 36. Deactivate React modals on accepted logical close — critical bounded design then current-owner implementation, part ofTask31. Native12/12fail acrossfourfamilies/threeengines while exit DOM remains: stale aria-modal/interactive focus,3families retainedguards, extra closed Escape callbacks. Existingreturnfocus/scroll release alreadypass. Accept inactive exit presentation, no stale callbacks/guards, preservedreopening/SSR/inline/pointer/native behavior; no broader registry/inert/dynamic qualification. Exactscope follows .batuta/v1-modal-logical-close-diagnosis.md.
