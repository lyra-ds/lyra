...
- 2026-09-19: after the squash merge, `main` no longer contained the branch source revision and its CI went red on the ancestry checks; the ledger was rebound to the published `main` commit `cd282f198151a7901aad86ff837c4367e9f163cd` (identical archives), FileUpload evidence re-produced there, gate re-pinned, core evidence moved. Mechanism lesson: under squash merges the candidate `sourceRevision` must be a `main` commit.
