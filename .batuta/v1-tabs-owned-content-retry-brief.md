This is the single retry of the existing high Tabs cycle. Apply the specific feedback below to the current uncommitted work; do not reset or reimplement unrelated owners. Feedback scope extension supersedes the initial generator-read-only sentence for exactly the named count/history change. No design/config/workflow interview or hook action is authorized.
...
## Scope
- tools/docgen/generate.mjs (only existing Props count/history)
- apps/site/scripts/landing-sections.test.mjs (only documentedComponents expectation)
- apps/site/lib/generated/stats.json (controller regeneration only)
