...
## Controller review — one bounded same-lane retry
Initial source10 tests each engine, SSR2, types/build/docgen/MDX/stack and public type negative proof passed. Native65/66 passed with no errors; sole WebKit ShiftTab checkbox expectation is a controller harness preference issue, not product work. Preserve runtime source and SSR bytes. Fix these review findings only within existing Scope:
1. In DataTableBasic and both ProjectTable snippets, the current status says details are open but contains no details. Make the consumer action actually display the selected project name and owner below the table, with polite announcement and truthful wording. Preserve projects, buttons, IDs and layout conventions. No new styling or component API.
2. Both MDX pages inaccurately call the old contract a pre1 exception. The removal uses the project's pre1 exception for unsafe contracts. State that accurately, manual migration, earliest correction0.6.0 or stable1.0.0. Include the compatibility exception and release timing in the minor changeset; no claim already published.
3. Browser stale-prop test mocks all console.error and only asserts one expected call exists. This can hide unrelated diagnostics. Account for exactly the expected unknown-event warning and ensure any other error causes failure. Preserve negative behavior assertions and no general console suppression.
Keep all other implementation intact. No tests/build/commit/delegation. Report exact changes and unrun checks.
