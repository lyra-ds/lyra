...
The old 38-task loop and its contract are now historical and suspended; no old-loop, container prerequisite, or proposed replacement task is executable from this documentation change. `.batuta/plans/v1-product-review.md` maps every old task to conditional reusable evidence, a concrete pending follow-up, or an explicit decision. It starts only one bounded prospective slice—implementing then running a packed React BottomSheet P1 producer—and stops afterward. It introduces no audit runner, check, registry, validator, security-portability work, threshold, baseline, or ledger. Old task 37's 11 P1 × 23 acceptance obligation remains pending in existing acceptance records and focused-report mapping until real coverage is checked; this is not a new registry, validator, or canonical-ledger promotion.

Native evidence is macOS-only: `pnpm test` 0/149.03s, `pnpm build` 0/50.22s, and the three-engine browser matrix 0/595.43s (3,684 cases). Linux and Windows native runs, remote workflow execution, and V1 artifact/platform qualification remain pending. Dialog/Drawer 18/18 and named React18/19 P1 evidence are reusable only where exact source, packed-artifact, tool, and environment identities apply. Blade, manual/touch requirements, immutable baselines, the three accepted composition exceptions, and future ceilings remain preserved.

## Final controller disposition

All four approved simplification tasks are complete. Task4: Terra medium, one documentation retry; conductor normalized remaining non-executable proof placeholders into semantic acceptance and retained explicit six-profile/three-engine requirements. Full Batuta structural/input-hash checks and exact38-ID mapping pass. GLM independent final review3/3DONE, unchanged source tree. Documentation-only task did not rerun passing product suites. Historical oldloop approval removed from parser metadata and operator commands disabled. Replacement remains proposed, bounded to one BottomSheet producer/review task then stop, with every remaining obligation preserved.

No active suite or dependency was deleted; only suspended experiment inclusion in the common gate changed. All product code, old required CI, routing, resource settings, lockfiles and immutable baselines remain unchanged. Native Windows/Linux results and remote CI remain pending. Full V1 qualification remains open.
