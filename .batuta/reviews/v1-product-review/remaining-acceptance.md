...
The [approved Tabs cap](tabs-approved-cap.md) and [seven approved migration exceptions](approved-migration-caps.md) now pass the complete native gate: 72 standalone entries, five scenarios and four CSS entries. All fresh archive identities and shipped code are unchanged. The recorded size blockers are closed; historical failures remain intact. The earlier [local qualification](final-local-qualification.md) remains valid within its exact code/artifact scope.
...
| --- | --- | --- |
| Final current-artifact qualification — local | Completed for the recorded package checks and exact artifact set, including the full native budget gate. Reuse these proofs while the shipped bytes remain identical. | All applicable checks pass; fresh exact artifact identities agree. Reused evidence has explicit byte/owner/environment applicability. No historical measurement is relabeled as the new package pair. |
