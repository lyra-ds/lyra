# Shared brief — @lyra-ds/alpine wave D (lots D1–D4 sit on top of this)
...
- `packages/alpine` — the package you are working on. Version 0.1.1 plus
  waves B and C merged on main: SEVENTEEN shipped bindings plus the
  `theme` store (dropdown, dialog, drawer, tabs, accordion, tooltip,
  popover, code-block, cookie-banner, sidebar-group, segmented-control,
  workspace-switcher, bottom-sheet, table-of-contents, time-input,
...
- Shared logic lives in `packages/alpine/src/internal/` as plain functions:
  ...
  `when-visible.ts`, `test-axe.ts`, `tooltip-placement.ts`,
  `scroll-spy.ts` already exist —
...
inventing your own: `src/bottom-sheet.ts` (presence/`mounted`, focus trap,
drag guard), `src/dropdown.ts` (outside mousedown, flip), `src/tabs.ts`
(roving tabindex, `:class` object syntax), `src/workspace-switcher.ts`
(pendingFocus, listbox), `src/file-upload.ts` (x-for over served template,
consumer-template helpers), with their test suites.

Two wave-C additions, hard-won:
