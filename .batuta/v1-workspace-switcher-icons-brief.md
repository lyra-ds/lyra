# workspace-switcher icon optimization

## Goal
Use the exact three existing Lucide glyphs for WorkspaceSwitcher private fixed icons without retaining the global Icon registry. No changes to public Icon/API, interactions or other components.
## Context
...
## Acceptance criteria
1. ChevronsUpDown, Check and Plus retain exact SVG/class/decorative/dimension/color output. Preserve selection, controlled/uncontrolled state, roving focus, native Tab/Enter/Space/Escape, click cancellation and creation operation. No public API change. Controller checks original-versus-final actual SVG DOM and visual snapshots across three engines, both themes and supported states.
...
## Scope
packages/react/src/workspace-switcher/workspace-switcher.tsx; packages/react/src/workspace-switcher/workspace-switcher.browser.test.tsx; packages/react/src/workspace-switcher/workspace-switcher.ssr.test.ts; .changeset/workspace-switcher-private-icons.md. Do not change anything outside this list; if the task requires it, stop and report.
## Expected evidence
...
## Dispatch precision
Controller has already resolved routing/profile/conventions above. Read only this brief, scoped component/tests, existing Icon implementation/registry glyph anchors and package metadata needed to confirm pinned tools. Do not sweep raw logs, other Batuta history, git history, unrelated components or the rest of the repository. No nested delegation. This explicit exception permits the exact pinned Prettier --write command on the scoped runtime/changeset files before returning. No other validation/build/package-manager commands.
