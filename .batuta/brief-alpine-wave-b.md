# Shared brief — @lyra-ds/alpine wave B (lots B1–B5 sit on top of this)
...
- `packages/alpine` — the package you are working on. Version 0.1.1, green,
  with SEVEN shipped bindings: dropdown, dialog, drawer, tabs, accordion,
  tooltip, popover. ESM-only tsdown build, single `.` export
  (`src/index.ts`) of an Alpine plugin, Browser Mode vitest harness
  (tests match `src/**/*.browser.test.ts`), size-limit budget in
  `package.json`. `alpinejs` is a peer dependency (devDep available for
  tests).
- `packages/react` — the SOURCE OF TRUTH for behavior. Each Alpine component
  ...
  classes, same ARIA, same focus/keyboard handling. Read the component's
  `.tsx` and its `.browser.test.tsx` before writing code.
...
put it. React callbacks become custom events via `this.$dispatch('lyra:...')`
(bubbling, with a detail payload).
</context>
...
- Shared logic lives in `packages/alpine/src/internal/` as plain functions:
  ...
  REUSE them; do not fork variants.
- Every controllable state must work with `x-modelable`: hold the state as a
  plain reactive property named exactly as the lot specifies, mutated only
  via plain assignment, and prove it in a test whose fixture uses
...
- The size-limit budget in `packages/alpine/package.json` will be exceeded
  ...
  round generous number.
- Register the component in `src/index.ts` inside the plugin function
  (`alpine.data('lyraX', ...)`), following the existing pattern.
</conventions>

<lessons_from_wave_1>
Hard-won across lots 02–08 and the 0.1.1 patch — each cost a failed round
or a shipped bug when ignored:
...
6. Test-construction rules: assertions after a synthetic `dispatchEvent`
   ...
   target. Placement-flip fixtures must FORCE the scenario arithmetically,
...
inventing your own: `src/dropdown.ts` (bindings, init/destroy, outside
mousedown, flip), `src/dialog.ts` (presence/`mounted`, focus trap),
`src/tabs.ts` (roving tabindex, `:class` object syntax), `src/accordion.ts`
(disclosure), with their test suites.
</lessons_from_wave_1>
...
<environment_limits>
Your sandbox CANNOT run Vitest Browser Mode (no localhost bind). Do not
try; do not report test results you could not produce. Instead:

- run `pnpm --filter @lyra-ds/alpine run typecheck` and
  `pnpm exec prettier --check packages/alpine` and report their real output;
- write the browser tests so the maestro runs them; structure test files so
  a syntax/nesting error is impossible to miss (no `it()` inside `it()`).
  </environment_limits>
...
contradicts this brief; the same command fails twice; the work would
