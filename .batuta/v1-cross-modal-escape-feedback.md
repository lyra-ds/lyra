...
## Single medium retry — tests only
Controller current Chromium/SSR124, WebKit110, Firefox110 and types/format/build/docgen PASS. Runtime is correct; preserve BOTH runtime files and changeset byte-for-byte. Lint fails jsx-a11y/no-static-element-interactions at48/82/107 in BottomSheet test and48/82/105 in Drawer test. Set role="presentation" on the six new ancestor fixture divs (established local test precedent); no suppressions/config changes.
The six new direct action.focus()/editor.focus() preparations also violate the requested real opening-focus path. For each fixture, use a normal native input aria-label="Editor" as the body test control. Existing modal opening focuses its header close control. Reach Editor using actual userEvent Tab, then assert document.activeElement is the exact input; never focus that input directly. A body button without explicitTabindex is not a stable native stop on this WebKit setup, so use the meaningful Editor input.
Account exactly for the legitimate Tab preparation event: in the first ordered-calls test assert the initial consumer-Tab and parent-Tab pair, then clear that precondition log before existing Escape/Enter assertions. In the stopPropagation-only fixture Tab is stopped by the existing consumer, so parent remains uncalled. In the descendant-cancel fixture the parent sees one Tab before focus reaches Editor; assert it once and then keep the exact count unchanged after Escape (or explicitly clear only after proving that precondition). Preserve the existing Escape close/no-close and propagation assertions, no permissive counts or hidden failures. No new tests/owners needed. Only the two colocated browser files may change; scoped exact pinned formatter allowed. Controller owns all checks.
