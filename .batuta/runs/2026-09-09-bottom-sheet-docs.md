...
## Executor report
Implemented the BottomSheet docs/examples update.

- Both examples now pass their Button refs via `returnFocusTo`.
- EN and pt-BR pages document resolver eligibility, fallback/ignored behavior, and successor responsibility, with a complete React snippet.
- Generated outputs were not edited.

Checks passed: `git diff --check`, Prettier. ESLint passed for TSX; MDX files are ignored by its config (warnings only).

Controller still needs build/types, browser focus proof, and docgen/check for generated outputs.

## Retry report
Applied only the two requested documentation corrections:

- Both BottomSheet pages now state ignored `onClose` requests do not resolve or move focus, and closed-state rerenders do not restore focus again.
- The updated pt-BR focus/scroll-lock bullet is entirely English.

Changed:

- [English BottomSheet docs](/Volumes/Home/francisross/Projects/lyra/lyra-v1-stabilization/apps/docs/content/docs/en/components/bottom-sheet.mdx)
- [pt-BR BottomSheet docs](/Volumes/Home/francisross/Projects/lyra/lyra-v1-stabilization/apps/docs/content/docs/pt-BR/components/bottom-sheet.mdx)

Verified: pinned Node Prettier `--check` passed; `git diff --check` passed. No commit or other changes made.
