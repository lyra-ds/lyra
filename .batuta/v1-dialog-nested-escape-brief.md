...
Controller trusted StrictMode Chromium/WebKit proof .batuta/runs/v1-dialog-nested-escape/red.json: all10 scenarios fail because parent onClose fires on child Escape. Cases: normal, closeOnEsc=false, panel consumer preventDefault, child input preventDefault, ignored child close request. Normal closes both; disabled still closes parent; prevented still closes both. Correction after reading the retained baseline JSON: Chromium basic child Tab/Shift+Tab passes; WebKit forward Tab reaches BODY while reverse Tab returns to the input on both baseline and current source. This separate focus-containment defect remains unqualified; Escape criteria are evaluated independently without suppressing that observation. There are no page errors. Fixtures use existing returnFocusTo refs, no mouse pre-focus workaround.
