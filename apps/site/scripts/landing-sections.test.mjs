...
test('exports bilingual privacy pages with one heading and the complete privacy copy', () => {
  for (const [
    ...
    [
      ...
      "Both lyra-ds.dev and docs.lyra-ds.dev are served by hosting providers. Like every web server on the internet, their providers record the requests they answer: your IP address, the time, the page and your browser's user agent. We do not control those logs and do not use them to identify anyone. They are the only places where data about your visits exists at all, and saying otherwise would be untrue.",
