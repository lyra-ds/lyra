...
Omit `mark` and Brand draws an initial-letter mark from the wordmark text (or from `aria-label` for
a mark-only brand), so a bare `<Brand>Lyra</Brand>` renders without an image.
