...
Workspace options now live in an inner listbox, with a native “Create workspace” command beside it.
Update direct-child selectors: `.lyra-wssw__pop > button` no longer matches the workspace options;
use `.lyra-wssw__pop .lyra-wssw__item` to match options and the create command.
