...
## Use caixa de frase nos rótulos

Rótulos em estilo overline usam caixa alta e seu espaçamento original por padrão. Para uma marca
ou localidade que usa caixa de frase, adicione os tokens tipográficos opcionais no mesmo escopo.
Eles afetam cabeçalhos de tabela, grupos da barra lateral e de menus, rótulos de calendário e
outros rótulos overline:
...
`--overline-transform: none` preserva a caixa escrita no HTML; escreva os rótulos em caixa de
frase (por exemplo, `Status da entrega`). Esses tokens não fazem parte dos quatro tokens base
da marca nem são derivados automaticamente de `--brand`.
...
Se você também personalizar `--danger` para uma cor clara, defina `--on-danger` com uma tinta mais
escura: preenchimentos danger sólidos, como `.lyra-btn--danger`, o usam e o padrão é `#FFFFFF`.
