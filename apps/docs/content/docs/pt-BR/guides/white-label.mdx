...
em `srgb` e deriva `--accent-soft`, `--accent-soft-text` e `--focus-ring` com `color-mix()` em
`oklab`. Ele também define `--on-accent`, `--text-link`, `--border-accent`, `--radius-md`,
`--font-sans` e `--font-display`.

Dentro de `data-theme="dark"`, as misturas da marca para accent, hover, active, soft-text e link usam branco.
