...
recalcula o grupo de acento a partir dessa semente, então os componentes dentro dessa subárvore
...
`--brand` é a semente obrigatória. Os outros três tokens têm defaults, mas defina
`--brand-contrast` sempre que a semente for clara o bastante para deixar texto branco ilegível.

| Token              | Default                                      | Finalidade                                   |
| ------------------ | -------------------------------------------- | -------------------------------------------- |
| `--brand`          | Obrigatório                                  | A semente da cor da marca.                   |
| `--brand-contrast` | `#FFFFFF`                                    | Texto e ícones sobre o acento primário.      |
...
No modo claro, o Lyra mantém `--accent` como o `--brand` cru, escurece hover e active com preto e
deriva `--accent-soft`, `--accent-soft-text` e `--focus-ring` com `color-mix()` em `oklab`. Ele
também define `--on-accent`, `--text-link`, `--border-accent`, `--radius-md`, `--font-sans` e
`--font-display`.

Dentro de `data-theme="dark"`, as misturas de accent, hover, active, soft-text e link usam branco.
...
`--on-accent` usa `#FFFFFF` por default. Isso funciona para uma marca escura, mas uma marca clara
precisa de um `--brand-contrast` escuro explícito; caso contrário, o rótulo de um botão primário
pode ficar ilegível. Confira o contraste de cada par marca/contraste antes de publicá-lo.
