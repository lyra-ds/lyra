...
status consistentes entre componentes. O texto sobre um preenchimento `--danger` sólido (por exemplo
`.lyra-btn--danger`) usa `--on-danger`, `#FFFFFF` por padrão. Sobrescreva-o quando um `--danger`
personalizado for claro demais para branco.
