...
`.lyra-tzpicker` é um hook de seleção, sem estilo próprio. Ela marca a raiz do seletor (junto de `.lyra-combobox`, que carrega toda a aparência) para que o seu CSS e scripts possam mirá-la, como o `SlotPicker` faz internamente. Use `className` para estendê-la em vez de reestilizar `.lyra-combobox`.
