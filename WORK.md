...
- [x] Calendar native Tab fixtures verified — OpenCode GLM/low, no retry; Chromium/SSR10, WebKit8, Firefox8 and static gates PASS; both exact entry tests fail with the active-day tab stop disabled and pass after restoration. Runtime unchanged. Commit pending; see .batuta/v1-calendar-tab-fixtures-verification.md.
- [ ] Continuous completion authorized on 2026-09-09. BottomSheet runtime/examples/docs complete; PR220 action update verified; React baseline is now765/765 after the independent WorkspaceSwitcher contrast repair, before selected11 library updates; dependency PR triage recorded. Follow .batuta/plans/v1-completion.md. Selected11 updates verified;9 source-size overages attributed; final packed budget decision pending. Dropdown keyboard contract is complete; WebKit focus correction tranche is in progress after the measured six-failure baseline.
