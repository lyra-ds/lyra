...
- [x] Nested Dialog Escape containment verified — Codex gpt-5.6-terra/medium, one formatting-only retry, no escalation; native Escape10/10, final Chromium/SSR64 and WebKit57, types/lint/format PASS. Independent GLM3/3 DONE, unchanged guard; final-byte verification observation resolved by reruns, controller WORK ownership concern declined. Implementation commit PENDING_COMMIT; see .batuta/v1-dialog-nested-escape-verification.md. Existing WebKit forward-Tab focus escape remains a separate pending slice.
- [ ] Remaining: other incumbent P1/modal contract slices and full packed Firefox/Linux release qualification. No executor/browser/server remains running; no publication performed.
