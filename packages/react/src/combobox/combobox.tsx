 ...
 */
export const Combobox = /*#__PURE__*/ forwardRef<HTMLButtonElement, ComboboxProps>(function Combobox(
  {
    label,
    hint,
    error,
  ...
  const filtered: IndexedOption[] = options
    ...
    .filter(({ option }) => !query || option.label.toLocaleLowerCase().includes(query.toLocaleLowerCase()));
  ...
  const handleSearchKeyDown = (event: KeyboardEvent<HTMLInputElement>): void => {
    if (event.key === 'ArrowDown') {
      ...
      if (filtered.length > 0) setActiveIndex((index) => Math.min(index + 1, filtered.length - 1));
          ...
          <div ref={listRef} id={listboxId} className="lyra-combobox__list" role="listbox">
            ...
            {filtered.map(({ option, index }, filteredIndex) => (
              <button
                ...
                className={cx('lyra-combobox__option', filteredIndex === activeIndex && 'lyra-combobox__option--active')}
              ...
              >
                ...
                <span className="lyra-combobox__option-label">
                  {option.label}
                  {option.hint && <span className="lyra-combobox__option-hint">{option.hint}</span>}
                </span>
                {option.value === current && <Icon name="check" size={15} color="var(--accent)" />}
              </button>
            ))}
          </div>
        </div>
  ...
  return (
    <div className="lyra-field">
      {label && <label id={labelId} className="lyra-label" htmlFor={triggerId}>{label}</label>}
      {control}
      {error ? (
        <span id={messageId} className="lyra-hint lyra-hint--error">{error}</span>
      ) : hint ? (
        <span id={messageId} className="lyra-hint">{hint}</span>
      ) : null}
    </div>
  );
});
