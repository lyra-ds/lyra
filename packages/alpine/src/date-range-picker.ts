...
/** Initial configuration accepted by `x-data="lyraDateRangePicker(...)"`. */
export interface LyraDateRangePickerOptions {
  /** Initial ISO `YYYY-MM-DD` bounds. */
  defaultValue?: { start?: string | null; end?: string | null } | null;
  /** BCP 47 locale used for the trigger's selected-date text. Default: `"en-US"`. */
  locale?: string;
  /** Trigger text when no range has started. Default: `"Select period"`. */
  placeholder?: string;
  /** Text between formatted range bounds. Default: `" – "`. */
...
/**
 * A thin date-range-picker coordinator. It owns only `selected`, `open`, responsive branch
 * selection, and the complete-range close rule; Popover, BottomSheet, and Calendar retain their
 * own bindings. It intentionally provides no `trigger` binding: the desktop trigger belongs to
 * the nested Popover and the mobile trigger uses one `@click="open = true"` handler.
 *
 * ```html
 * <div x-data="lyraDateRangePicker({ defaultValue: { start: '2024-05-01', end: null }, locale: 'en-US' })">
 ...
 *       </button>
 *       </div>
 *       <!-- Alias scope: same-name x-model chains must proxy the picker state. -->
 *       <div x-data="{ get pickerOpen() { return open }, set pickerOpen(v) { open = v } }">
 ...
 *           aria-labelledby="date-range-picker-sheet-title" x-bind="panel">
 ...
 * ```
 */
export function lyraDateRangePicker({
  defaultValue,
  locale = 'en-US',
  ...
  const normalizeRange = (value: unknown): { start: Date | null; end: Date | null } => {
    ...
    return { start: normalizeDay(range.start), end: normalizeDay(range.end) };
  };

  const state: LyraDateRangePickerData & ThisType<LyraDateRangePickerState> = {
    selected: normalizeRange(defaultValue),
    open: false,
    mobile: false,
    locale,
    placeholder,
    ...
    hasSelection() {
      return normalizeRange(this.selected).start !== null;
    },
  };
