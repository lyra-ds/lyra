...
- [ ] **Step 3: Run structural and selection checks**
  ...
  ```bash
  rtk node --input-type=module -e "import { readFileSync } from 'node:fs'; const file = 'docs/superpowers/backlog/2026-08-15-next-delivery-evidence.md'; const text = readFileSync(file, 'utf8'); const sections = text.split(/^## /m); const headings = ['Baseline','Evidence Sources','Candidate Evidence','Candidates','Scoring','Focused Audit','Recommendation','Next Delivery Contract','Verification']; const getSections = (heading) => sections.filter((section) => section.startsWith(heading + '\\n')); for (const heading of headings) if (getSections(heading).length !== 1) throw new Error('Expected exactly one heading: ' + heading); const candidatesSection = getSections('Candidates')[0]; const candidateRows = candidatesSection.split('\\n').filter((line) => /^\\|\\s+/.test(line)); const candidates = [...new Set(candidateRows.flatMap((line) => line.match(/\\bBKL-\\d{2}\\b/g) ?? []))]; if (candidates.length === 0) throw new Error('No candidate table row recorded'); const recommendation = getSections('Recommendation')[0]; const selectedLines = recommendation.match(/^\\*\\*Selected:\\*\\* BKL-\\d{2}$/gm) ?? []; if (selectedLines.length !== 1) throw new Error('Recommendation must contain exactly one selection'); const selected = selectedLines[0].match(/BKL-\\d{2}/)[0]; if (!candidates.includes(selected)) throw new Error('Recommendation must select one candidate-table identifier'); const classes = recommendation.match(/^\\*\\*Classification:\\*\\* (Confirmed|Rejected|Inconclusive)$/gm) ?? []; if (classes.length !== 1) throw new Error('Recommendation must contain one allowed classification');"
  ```
